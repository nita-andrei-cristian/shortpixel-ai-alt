...
class SPAATGSettings {
	...
	UpdateAiExampleEvent(response)
	{
			...
			{
				var img = document.querySelector('.preview_wrapper img.image_preview'); 
				img.src = json.preview_image;
			}

			for (let i = 0; i < elements.length; i++)
			{
				let name = elements[i]; 
				var dataObject = (name ==  'generated') ? generatedData : currentData;
				...
				if (typeof json[name] !== 'undefined')
				{
					...
					{
						let field = fields[j]; 
						let element = dataObject.querySelector('.' + field); 
						if (element !== null)
						{
							if (typeof data[field] === 'undefined' || null === data[field] || data[field].length === 0)
							{
								var val = '&nbsp;'; 
							}
							else
